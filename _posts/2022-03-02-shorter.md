...
Or, imagine a sine wave centered at the origin of the cartesian plane. The we take the integral from a to b, and that quantity tells us how many degrees to rotate the vector at a, to the right. If the definite integral is negative, we interpret that to mean applying the inverse rotation to the left from a to b. 
